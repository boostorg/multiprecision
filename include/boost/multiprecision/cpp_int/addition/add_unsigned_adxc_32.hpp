...
#ifndef BOOST_MP_ADD_UNSIGNED_ADXC_32_HPP
#define BOOST_MP_ADD_UNSIGNED_ADXC_32_HPP
...
inline BOOST_MP_CXX14_CONSTEXPR void add_unsigned(CppInt1& result, const CppInt2& a, const CppInt3& b) BOOST_MP_NOEXCEPT_IF(is_non_throwing_cpp_int<CppInt1>::value)
{
...
#endif
   {
      ...
      for (; i + 4 <= m; i += 4)
      {
         carry = _addcarryx_u32(carry, pa[i + 0], pb[i + 0], (unsigned int*)&pr[i + 0]);
         carry = _addcarryx_u32(carry, pa[i + 1], pb[i + 1], (unsigned int*)&pr[i + 1]);
         carry = _addcarryx_u32(carry, pa[i + 2], pb[i + 2], (unsigned int*)&pr[i + 2]);
         carry = _addcarryx_u32(carry, pa[i + 3], pb[i + 3], (unsigned int*)&pr[i + 3]);
      }
      for (; i < m; ++i)
         carry = _addcarryx_u32(carry, pa[i], pb[i], (unsigned int*)&pr[i]);
      for (; i < x && carry; ++i)
         carry = _addcarryx_u32(carry, pa[i], 0, (unsigned int*)&pr[i]);
      if (i == x && carry)
      {
