...
#ifndef BOOST_MP_ADD_UNSIGNED_ADDC_64_HPP
#define BOOST_MP_ADD_UNSIGNED_ADDC_64_HPP
...
inline BOOST_MP_CXX14_CONSTEXPR void add_unsigned(CppInt1& result, const CppInt2& a, const CppInt3& b) BOOST_MP_NOEXCEPT_IF(is_non_throwing_cpp_int<CppInt1>::value)
{
...
#endif
   {
      ...
      for (i = 0; i + 4 <= m; i += 4)
      {
         ...
         carry = _addcarry_u64(carry, pa[i + 2], pb[i + 2], (cast_type*)&pr[i + 2]);
         carry = _addcarry_u64(carry, pa[i + 3], pb[i + 3], (cast_type*)&pr[i + 3]);
      }
      for (; i < m; ++i)
         carry = _addcarry_u64(carry, pa[i], pb[i], (cast_type*)&pr[i]);
      for (; i < x && carry; ++i)
         carry = _addcarry_u64(carry, pa[i], 0, (cast_type*)&pr[i]);
      if (i == x && carry)
      {
