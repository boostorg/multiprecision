...
namespace boost {
    namespace multiprecision {
        namespace backends {
            ...
            public:
                ...
                modular_adaptor(const BackendBase &val, const montgomery_params <BackendMod> &mod) : m_base(val),
                        m_mod(mod) {
                }

                modular_adaptor(const montgomery_params <BackendMod> &mod) : m_base(0), m_mod(mod) {
                }
                ...
                std::string str(std::streamsize dig, std::ios_base::fmtflags f) const {
                    modular_adaptor<BackendBase, montgomery_params < BackendMod>>
                    tmp = this;
                    redc(tmp);
            ...
            template<class BackendBase, class BackendMod, class T> inline typename enable_if<is_arithmetic < T>, bool>

            ::type eval_eq(const modular_adaptor<BackendBase, BackendMod> &a, const T &b)
    ...
    inline typename disable_if_c<boost::is_complex<Result>::value>::type eval_convert_to(Result *result,
    ...
    inline void assign_components(modular_adaptor <BackendBase, BackendMod> &result, const T &a,
                                  const montgomery_params <V> &b) {
        result.base_data() = a;
        result.mod_data() = b;
    }
