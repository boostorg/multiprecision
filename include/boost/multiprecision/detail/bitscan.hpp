...
{
   return static_cast<std::size_t>(sizeof(unsigned) * CHAR_BIT - 1u - static_cast<std::size_t>(__builtin_clz(static_cast<unsigned int>(mask))));
}
BOOST_FORCEINLINE std::size_t find_msb(unsigned long mask, std::integral_constant<int, 2> const&)
{
   return static_cast<std::size_t>(sizeof(unsigned long) * CHAR_BIT - 1u - static_cast<std::size_t>(__builtin_clzl(static_cast<unsigned long>(mask))));
}
BOOST_FORCEINLINE std::size_t find_msb(unsigned long long mask, std::integral_constant<int, 3> const&)
{
   return static_cast<std::size_t>(sizeof(unsigned long long) * CHAR_BIT - 1u - static_cast<std::size_t>(__builtin_clzll(static_cast<unsigned long long>(mask))));
}
