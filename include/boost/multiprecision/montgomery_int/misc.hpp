...
inline
    typename enable_if_c<is_integral<R>::value &&
    ...
    eval_convert_to(R*                                                  result,
                    const montgomery_int_backend<MinBits1, MaxBits1, SignType1,
                                       ...
                                       Allocator1, ParamsBackend1> >::value))
{
   ...
   *result = static_cast<R>(backend.limbs()[0]);
   ...
   for (unsigned i = 1;
        (i < backend.size()) &&
        (shift < static_cast<unsigned>(std::numeric_limits<R>::digits));
        ++i)
   {
      *result += static_cast<R>(backend.limbs()[i]) << shift;
      shift += montgomery_int_backend<MinBits1, MaxBits1, SignType1, Checked1,
                                      Allocator1, ParamsBackend1>::limb_bits;
   }
   if (backend.sign())
   {
...
template <typename R, unsigned MinBits1, unsigned MaxBits1,
          cpp_integer_type SignType1, cpp_int_check_type Checked1,
          class Allocator1, typename ParamsBackend1>
inline
    typename enable_if_c<is_floating_point<R>::value &&
                             !is_trivial_montgomery_int<montgomery_int_backend<
                                 MinBits1, MaxBits1, SignType1, Checked1,
                                 Allocator1, ParamsBackend1> >::value,
                         void>

    ::type eval_convert_to(
        R*                                                        result,
        ...
        BOOST_MP_NOEXCEPT_IF(is_arithmetic<R>::value)
{
   typename montgomery_int_backend<MinBits1, MaxBits1, SignType1, Checked1,
                                   Allocator1,
                                   ParamsBackend1>::const_limb_pointer p =
       backend.limbs();
   unsigned shift =
       montgomery_int_backend<MinBits1, MaxBits1, SignType1, Checked1,
                              Allocator1, ParamsBackend1>::limb_bits;
                                     ...
                                     Allocator1, ParamsBackend1>& a)
{
   ...
   return result +
          index * montgomery_int_backend<MinBits1, MaxBits1, SignType1, Checked1,
                                         Allocator1, ParamsBackend1>::limb_bits;
}
...
inline typename enable_if_c<!is_trivial_montgomery_int<
    ...
    ::type
    eval_bit_set(montgomery_int_backend<MinBits1, MaxBits1, SignType1, Checked1,
                                        Allocator1, ParamsBackend1>& val,
                 unsigned                                            index)
{
   unsigned offset =
...
inline typename enable_if_c<!is_trivial_montgomery_int<
    ...
    ::type eval_qr(
        const montgomery_int_backend<MinBits1, MaxBits1, SignType1, Checked1,
                                     Allocator1, ParamsBackend1>& x,
        const montgomery_int_backend<MinBits1, MaxBits1, SignType1, Checked1,
                                     Allocator1, ParamsBackend1>& y,
        montgomery_int_backend<MinBits1, MaxBits1, SignType1, Checked1,
                               Allocator1, ParamsBackend1>&       q,
        montgomery_int_backend<MinBits1, MaxBits1, SignType1, Checked1,
                               Allocator1, ParamsBackend1>&       r)
...
template <unsigned MinBits1, unsigned MaxBits1, cpp_integer_type SignType1,
          cpp_int_check_type Checked1, class Allocator1,
          typename ParamsBackend1, class U>
inline typename enable_if_c<is_integral<U>::value>::type
eval_qr(const montgomery_int_backend<MinBits1, MaxBits1, SignType1, Checked1,
                                     Allocator1, ParamsBackend1>& x,
        U                                                         y,
        montgomery_int_backend<MinBits1, MaxBits1, SignType1, Checked1,
                               Allocator1, ParamsBackend1>&       q,
...
template <unsigned MinBits1, unsigned MaxBits1, cpp_integer_type SignType1,
          cpp_int_check_type Checked1, class Allocator1,
          typename ParamsBackend1, class Integer>
inline
    typename enable_if_c<is_unsigned<Integer>::value &&
                             !is_trivial_montgomery_int<montgomery_int_backend<
                                 MinBits1, MaxBits1, SignType1, Checked1,
                                 Allocator1, ParamsBackend1> >::value,
                         Integer>
...
template <unsigned MinBits1, unsigned MaxBits1, cpp_integer_type SignType1,
          ...
          typename ParamsBackend1, class Integer>
BOOST_MP_FORCEINLINE
    typename enable_if_c<is_signed<Integer>::value &&
                             !is_trivial_montgomery_int<montgomery_int_backend<
                                 MinBits1, MaxBits1, SignType1, Checked1,
                                 Allocator1, ParamsBackend1> >::value,
                         Integer>
   ...
   do
   {
    ...
    Let u = min(u, v), v = diff(u, v)/2. */
      if (u.size() <= 2)
      {
         ...
         {
            double_limb_type i;
            i = u.limbs()[0] | (static_cast<double_limb_type>(u.limbs()[1])
    ...
    Let u = min(u, v), v = diff(u, v)/2. */
      ...
      if (v.size() <= 2)
      {
         ...
         else
         {
            ...
            i = v.limbs()[0] | (static_cast<double_limb_type>(v.limbs()[1])
                                << sizeof(limb_type) * CHAR_BIT);
            j = (u.size() == 1)
                    ? *u.limbs()
...
{
   typedef typename common_type<R, typename montgomery_int_backend<
                                       MinBits1, MaxBits1, SignType1, Checked1,
        ...
        static_cast<common_type>((std::numeric_limits<R>::max)())))
   {
      if (val.isneg())
      {
         ...
         {
            conversion_overflow(
                typename montgomery_int_backend<MinBits1, MaxBits1, SignType1,
                                                Checked1,
   ...
   else
   {
      *result = static_cast<R>(*val.limbs());
      ...
      {
         check_is_negative(
             mpl::bool_ < is_signed_number<R>::value ||
...
template <unsigned MinBits1, unsigned MaxBits1, cpp_integer_type SignType1,
          ...
          typename ParamsBackend1>
inline std::size_t
hash_value(const montgomery_int_backend<MinBits1, MaxBits1, SignType1, Checked1,
                                        Allocator1, ParamsBackend1>& val)
    BOOST_NOEXCEPT
{
   std::size_t result = 0;
