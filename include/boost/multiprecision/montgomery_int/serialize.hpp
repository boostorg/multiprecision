...
template <typename T> struct is_binary_archive : public mpl::false_ {};
template <>
...
void serialize(Archive &ar,
  ...
  typedef mpl::bool_<mp::backends::is_trivial_cpp_int<
      mp::montgomery_int_backend<MinBits, MaxBits, SignType, Checked, Allocator,
                                 ParamsBackend>>::value>
      trivial_tag;
  typedef typename montgomery_int_detail::is_binary_archive<Archive>::type
      binary_tag;

  // Just dispatch to the correct method:
  montgomery_int_detail::do_serialize(ar, val, save_tag(), trivial_tag(),
                                      binary_tag());
}
