//---------------------------------------------------------------------------//
// Copyright (c) Nil Foundation AG
// Copyright (c) Mikhail Komarov <nemo@nilfoundation.org>
// Copyright (c) Alexey Moskvin
//
...
void eval_mask_bits(Backend& val, Integer n)
{
   ...
   static const ui_type one = 1u;
   Backend tmp;
   backends::eval_bit_set(tmp, n);
   backends::eval_subtract(tmp, one);
   backends::eval_bitwise_and(val, tmp);
//   typedef typename boost::multiprecision::detail::canonical<unsigned, Backend>::type ui_type;
