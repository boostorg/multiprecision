...
namespace nil {
    namespace crypto3 {
        ...
        inline Backend eval_almost_montgomery_inverse(Backend &result, const Backend &a, const Backend &b) {
        ...
        inline number<Backend, ExpressionTemplates> almost_montgomery_inverse(
                number<Backend, ExpressionTemplates> &result, const number<Backend, ExpressionTemplates> &a,
                const number<Backend, ExpressionTemplates> &b) {
            return number<Backend, ExpressionTemplates>(
                    eval_almost_montgomery_inverse(result.backend(), a.backend(), b.backend()));
        }
         ...
         */
        template<typename Backend>
        inline Backend eval_normalized_montgomery_inverse(const Backend &a, const Backend &p) {
            using default_ops::eval_integer_modulus;
         ...
         */
        template<typename Backend, expression_template_option ExpressionTemplates>
        inline number<Backend, ExpressionTemplates> normalized_montgomery_inverse(
                const number<Backend, ExpressionTemplates> &a, const number<Backend, ExpressionTemplates> &p) {
            return number<Backend, ExpressionTemplates>(eval_normalized_montgomery_inverse(a.backend(), p.backend()));
        }
    }
}
